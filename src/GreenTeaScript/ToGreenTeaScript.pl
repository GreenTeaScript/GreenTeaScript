...
sub GtConstSection{
	my $text = $_[0];
	$text =~ s/(?:$Attr*)($Type)\s+($Sym)((?:\[\s*\d*\s*\])?)/let $2$3/g;
	$text =~ s/$Attr//g;
	return $text;
}
...
sub GtStaticSection{
	my $text = $_[0];
	$text =~ s/$Attr*//g;
	#$text =~ s/($Attr*)($Type)\s+($Sym)\s*\((.*?)\)/ $1$3(4): $2/g;
	return $text;
}
...
$src =~ s/\/\*field\*\/($Attr*)($Type)\s+($Sym)/$1$2$3/gm;
# Local variables.  int n = 0; => var n: int = 0;
$src =~ s/\/\*local\*\/($Type)\s+($Sym)/$1 $2/g;
# Constractors.
$src =~ s/($Attr*)($Sym)\/\*constructor\*\/\((.*?)\)/$1constructor($3)/g;
...
$src =~ s/($Attr*)($Type)\s+($Sym)\s*\((.*?)\)/\@Export $1$2 $3($4)/g;
# Method's parameters.
...
# Types
$src =~ s/(?!")\b(?:char|int|long|Charactor|Integer|Long)\b(?!")/int/g;
$src =~ s/(?!")\b(?:float|double|Float|Double)\b(?!")/float/g;
